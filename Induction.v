...
Proof.
  ...
  -(*n = 0*) simpl. induction m as [|m' IHm']. 
                    ...
                    (*m = S m*) simpl. rewrite IHm'. reflexivity. 
  -(*n = S n'*) simpl. rewrite <- IHn'. rewrite <- plus_n_Sm.
                reflexivity. Qed.
...
Definition manual_grade_for_destruct_induction : 
...
Theorem zero_eqb_n_mult_0 : forall n:nat, 0 = n * 0.
Proof.
  intros n. 
...
Theorem Sm_mult_n_eqb_m_mult_n_plus_n : 
forall n m : nat, (S m) * n = (m*n) + n.
Proof.
  intros. induction n.
          - (*n=0*) simpl. rewrite plus_comm. rewrite plus_O_n. reflexivity.
          - (*n = S n*) simpl. 
Admitted. 



Theorem m_mult_n_plus_n_eqn_n_mult_Sm : 
forall n m : nat, (m*n) + n = n * S m.
Proof.
  intros. induction n.
          - (*n=0*) simpl. rewrite plus_comm. rewrite plus_O_n.
...
Proof.
 intros m n.
  ...
  simpl.
  assert (H1: 0 = n * 0).
  {rewrite <- zero_eqb_n_mult_0. reflexivity. }
  apply H1.
  - (* m = S m' *)
  assert ( H2: (S m') * n = (m'*n) + n ).   (* (S m') * n = (m'*n) + n *)
  {rewrite <- Sm_mult_n_eqb_m_mult_n_plus_n. reflexivity. }
  rewrite H2.
  assert ( H3: (m'*n) + n = n * S m').      (* (m'*n) + n = n * (S m') *)
  {rewrite <- m_mult_n_plus_n_eqn_n_mult_Sm. reflexivity. }
  rewrite H3.
  reflexivity.
Qed.
...
Theorem distribute:  forall n m : nat, n + m * n = n * (1 + m).
 Proof.
    intros. induction n. 
            - (*n=0*) simpl. rewrite <- zero_eqb_n_mult_0. reflexivity.
            - (* n = S n*) simpl. 
 Admitted.
...
Proof.
  intros. induction m.
          - (* m=0 *) simpl. rewrite <- zero_eqb_n_mult_0. reflexivity.
          - (* m=S m' *)simpl.  rewrite <- plus_1_l. rewrite distribute. reflexivity.
Qed.
...
Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  |Z => O
  |A b => mult (S (S O)) (bin_to_nat b)
  |B b => S (mult (S (S O)) (bin_to_nat b))
end.
...
Theorem incr_then_bintonat_eqb_bintonat_then_succ : 
forall m : bin, bin_to_nat( incr(m)) = S (bin_to_nat(m)).
...
(* Wed Jan 9 12:02:44 EST 2019 *)
    ...
    Theorem n_eqb_n: forall n:nat, true = (n =? n).
